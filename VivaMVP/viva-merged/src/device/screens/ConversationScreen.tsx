...
import type { RobotMicAdapter } from '../../hooks/useVoiceInput';
import {
...
import { speak, stopSpeaking, setAudioSink, TTS_PAUSE_MARKER } from '../../services/tts.service';
...
interface ConversationScreenProps {
  ...
  /** Pi 가 살아 있는지 (App 의 /health 판정). 오디오 라우팅은 이걸 따른다 -
   * photoSource 는 "사진을 어디서 찍었나"만 뜻한다. 폰 카메라 폴백 세션에서도
   * 스피커·마이크가 살아 있으면 로봇으로 가야 한다 (실기기 2026-08-04). */
  robotAudio?: boolean;
  solveMode: boolean;
...
export default function ConversationScreen({
  ...
  // 로봇 모드 = Pi 가 살아 있는 세션. Pi 가 살아 있으면 TTS 는 로봇 스피커로,
  // STT 는 로봇 마이크로. 예전엔 photoSource==='pi' 로 판정했는데(D-33), 폰
  // 카메라 폴백을 타면 사진 출처가 'phone' 이 되면서 멀쩡한 로봇 스피커·
  // 마이크까지 폰으로 굳는 버그가 있었다 (D-33 갱신, 2026-08-04).
  const robotMode = robotAudio;
  useEffect(() => {
    setAudioSink(
      robotMode
        ? {
  ...
  const disconnected = piStatus === 'disconnected';
  // customSpeak 는 useCallback 클로저라 state 를 직접 읽으면 stale - ref 로 미러.
  ...
  const customSpeak = useCallback(
    async (text: string, _onPlay?: (d: number) => void, rate?: number) => {
      // 끊김 중 도착한 Gemini 응답이 폰 스피커로 재생되면 안 된다 (스펙 원칙:
      // 끊김 시 조용한 폰 폴백 금지). 호출부는 이 Promise 를 await 하므로
      // no-op resolve - throw 하면 FSM 호출 체인이 깨진다.
      ...
      // ANSWER_CONFIRMATION_POLICY) to synthesize a real pause. buildSubtitleSchedule
      // (called inside startSubtitles) now wants that SAME raw token too - it
      // counts markers per sentence to weight the pause into that sentence's
      // display time (each marker = a real 450ms SSML break), then strips it
      // to a space itself before the sentence ever reaches the screen. Only
      // the char-length fallback estimate below needs the stripped length.
      const subtitleText = text.split(TTS_PAUSE_MARKER).join(' ');
      await speak(
        text,
        (durationMillis) => {
          startSubtitles(text, durationMillis || subtitleText.length * SUBTITLE_MS_PER_CHAR);
        },
        rate,
      );
  ...
  const {
    ...
    fetchProblemCropFn: fetchPiPhotoCropBase64,
    cropLocalImageFn: cropBase64Image,
    recapturePiRegionFn: recapturePiRegion,
    fetchPiPhotoFn: fetchPiPhotoBase64,
    capturePhotoNowFn: capturePhotoNow,
    prewarmPiFocusFn: prewarmPiFocus,
  });
  ...
  // 훅은 Pi 를 모른다 - robotMode 일 때만 Pi 브릿지 함수 4개를 어댑터로
  // 묶어 넘긴다(화면 분리, Task 3). robotMode 가 꺼지면 null 이라 훅은
  // 그냥 폰 마이크 경로를 탄다.
  ...
  const voice = useVoiceInput({
    ...
    // 디바이스 연동판: 마이크는 로봇 전용이다 - 로봇 마이크 시작이 실패해도
    // 폰 마이크로 조용히 새지 않는다 (스펙 원칙, Task 6).
  ...
  useEffect(() => {
    ...
    // 이 시점엔 AudioSink/RobotMicAdapter 가 이미 해제(null)돼 위의
    // stopSpeaking/voice.stopListening 은 Pi 에 아무 신호도 못 보낸다 - Pi 쪽
    // 재생/녹음을 직접 끊어야 끊김 순간 로봇이 계속 말하거나 듣는 상태로
    // 남지 않는다.
  ...
  useEffect(() => {
    const shouldAutoListen =
      ...
      // 끊김 중엔 폰 마이크를 자동으로 열면 안 된다 (조용한 폰 폴백 금지).
      !disconnected;
    if (shouldAutoListen) {
  ...
  useEffect(() => {
    if (conversation?.initialAnalysis) {
      ...
      startSession(
        ...
        conversation.parentConceptSessionId,
      );
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [conversation]);
  ...
  // Mirror the eye state to the physical display board. This must live here
  // (not in CharacterView) because BoardView replaces CharacterView for most
  // of the session - the board's eyes froze on the last pre-board state.
  useEffect(() => {
    eyeSyncService.sendEyeState(eyeState);
  }, [eyeState]);
  // 언마운트 시 idle 전송은 useAppState.transitionTo 의 상태 미러링이 대신한다
  // - 여기서도 보내면 상태 전이가 이미 보낸 값을 한 박자 늦게 덮어쓴다.
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={undefined}>
      ...
      >
        ...
        {requiresBoard ? (
          ...
          // 자막은 판서 유무와 무관하게 아래 statusRow 의 하단 캡션으로만 낸다
          // (실기기 피드백 2026-08-11: 판서 없는 첫 호출/개념 답변에서 자막이
          // 중앙에 크게 떠 튜터링 루프의 하단 캡션과 따로 놀았다). CharacterView
          // 는 이제 자막을 그리지 않고 배경/마이크만 담당한다.
          <CharacterView
            state={eyeState}
            isListening={voice.isListening}
            onMicPress={handleMicPress}
        ...
        {disconnected && <DisconnectOverlay onEndSession={() => onSessionComplete?.()} />}
      </Pressable>
    </KeyboardAvoidingView>
  );
}
